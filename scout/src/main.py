...
def run() -> None:
    ...
    for x in range(0, 60):
        ...
        if rc_data[5] == 2000:  # flight mode on
            FATAL_ERROR(
                "Flight mode detected as on (from RC transmitter) as soon as power was received. As a safety precaution, mode switch needs to be in standby mode when system is powered up.")
        time.sleep(0.025)
    ...
    while ((time.ticks_ms() - started_at_ticks_ms) / 1000) < 3.0:
        ...
        gyro_z = (translate_pair(gyro_data[4], gyro_data[
            5]) / 65.5) * -1  # multiply by -1 because of the way I have it mounted on the quadcopter - it may be upside down. I want a "yaw to the right" to be positive and a "yaw to the left" to be negative.
        gxs.append(gyro_x)
        gys.append(gyro_y)
        gzs.append(gyro_z)
        time.sleep(0.025)
    ...
    cycle_time_us: int = int(
        round(cycle_time_seconds * 1000000, 0))  # multiply by 1,000,000 to go from seconds to microseconds (us)
    ...
    try:
        while True:
            ...
            gyro_data = i2c.readfrom_mem(mpu6050_address, 0x43, 6)  # read 6 bytes (2 for each axis)
            gyro_x = ((translate_pair(gyro_data[0], gyro_data[
                1]) / 65.5) - gyro_bias_x) * -1  # Roll rate. we multiply by -1 here because of the way I have it mounted. it should be rotated 180 degrees I believe, but it's okay, I can flip it here.
            gyro_y = (translate_pair(gyro_data[2], gyro_data[3]) / 65.5) - gyro_bias_y  # Pitch rate.
            gyro_z = ((translate_pair(gyro_data[4], gyro_data[
                5]) / 65.5) * -1) - gyro_bias_z  # Yaw rate. multiply by -1 because of the way I have it mounted - it may be upside down. I want a "yaw to the right" to be positive and a "yaw to the left" to be negative.
            ...
            # 1: roll 右摇杆的水平轴，控制滚动速率 (1000-2000)
            # 2: pitch 右摇杆的垂直轴，控制音调速率 (1000-2000)
            # 3  throttle 左摇杆的垂直轴，控制所有四个电机的平均推力（油门），控制四轴飞行器是上升还是下降 (1000-2000)
            # 4: yaw 左摇杆的水平轴。控制偏航率 (1000-2000)
            ...
            if rc_data[5] == 1000:  # standby mode - switch in "up" or OFF position
                ...
                if last_mode == False:  # last mode we were in was standby mode. So, this is the first frame we are going into flight mode
                    if input_throttle > 0.05:  # if throttle is > 5%
                        FATAL_ERROR("Throttle was set to " + str(
                ...
                print(
                    "Channel 5 input '" + str(rc_data[5]) + "' not valid. Is the transmitter turned on and connected?")
